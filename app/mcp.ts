...
// the native tandem binary (no Docker) and drive it — join a meeting, poll the
// transcript/participants, mute, leave — by speaking newline-delimited JSON-RPC
// over the child's stdin/stdout. The tandem stderr stream is captured as live logs.

import { loginPath, readConfig, tandemBinary, TANDEM_DIR } from "./backend.ts";
...
class MCPClient {
  ...
  #send(method: string, params?: unknown, notify = false): Promise<unknown> {
    ...
    // The write returns a promise; if the tandem child has exited its stdin pipe is
    ...
    this.#stdin.write(this.#enc.encode(line)).catch((e) => {
      ...
      if (id !== undefined) {
        ...
        if (pend) { this.#pending.delete(id); pend.reject(new Error("tandem engine exited: " + (e?.message ?? e))); }
      }
    });
    return p;
  }
  ...
  async initialize() {
    await this.#send("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "tandem-desktop", version: "0.1.0" },
    });
...
export async function joinMeeting(rawUrl: string, context = ""): Promise<{ ok: boolean; error?: string; result?: string }> {
  ...
  if (!bin) return { ok: false, error: "Tandem engine not built — open Settings and click Build now" };
  ...
  try {
    ...
    // tandem returns a plain-text result; a validation/refusal means we did NOT join,
