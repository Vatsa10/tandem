...
// we list upcoming events (including their Meet links so Tandem can join them).
//
// The credential setup in Cloud Console is fiddly, so `autoSetup()` can drive it
// with computer-use (the tandem binary's on-screen control); if that isn't
// available it opens the right console pages and returns step-by-step guidance.

import { openInShell, readConfig, writeConfig } from "./backend.ts";

const CAL_SCOPE = "https://www.googleapis.com/auth/calendar.readonly";
const AUTH_EP = "https://accounts.google.com/o/oauth2/v2/auth";
const TOKEN_EP = "https://oauth2.googleapis.com/token";

// Google OAuth "Desktop app" client. Provide your own via ~/.tandem/config.json
// (google_client_id / google_client_secret) or the TANDEM_GOOGLE_CLIENT_ID /
// TANDEM_GOOGLE_CLIENT_SECRET env vars — none is shipped in source, so calendar is
// opt-in with your own Cloud Console client (Application type: Desktop app).
const DEFAULT_CLIENT_ID = Deno.env.get("TANDEM_GOOGLE_CLIENT_ID") ?? "";
const DEFAULT_CLIENT_SECRET = Deno.env.get("TANDEM_GOOGLE_CLIENT_SECRET") ?? "";
...
export async function connectGoogle(): Promise<{ ok: boolean; error?: string }> {
  ...
  if (!clientId || !clientSecret) {
    return { ok: false, error: "No Google client configured. Add google_client_id / google_client_secret to ~/.tandem/config.json (create a Desktop-app OAuth client in Google Cloud Console)." };
  }
  ...
  const server = Deno.serve(
    ...
    (req) => {
      ...
      const html = (msg: string) =>
        new Response(
          `<!doctype html><meta charset=utf-8><body style="font:16px -apple-system,sans-serif;display:grid;place-items:center;height:100vh;margin:0;color:#2b2822"><div style="text-align:center"><h2>${msg}</h2><p style="color:#8a8378">You can close this tab and return to Tandem.</p></div>`,
          { headers: { "content-type": "text/html; charset=utf-8" } },
        );
      if (code) {
        resolveCode(code);
        return html("Tandem is connected to Google Calendar");
      }
  ...
  try {
    ...
    if (!r.ok || !tok.refresh_token) {
      return {
        ok: false,
        error: tok.error_description || tok.error || "token exchange failed (no refresh token — remove Tandem at myaccount.google.com/permissions and retry)",
      };
    }
