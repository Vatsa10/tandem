...
// (newline-delimited JSON-RPC 2.0). The connected Claude agent adds tandem as an
...
func (s *MCPServer) toolJoin(raw json.RawMessage) (string, bool) {
	...
	// Realtime mode (TANDEM_VOICE=realtime) uses OpenAI for STT+brain+TTS, so it needs
	...
	realtime := os.Getenv("TANDEM_VOICE") == "realtime"
	dgKey := os.Getenv("DEEPGRAM_API_KEY")
	if realtime {
		if os.Getenv("OPENAI_API_KEY") == "" {
			return "OPENAI_API_KEY not set (required for TANDEM_VOICE=realtime)", true
		}
	} else if dgKey == "" {
		return "DEEPGRAM_API_KEY not set (or set TANDEM_VOICE=realtime to use only OPENAI_API_KEY)", true
	}
	owner := a.Owner
	if owner == "" {
		owner = os.Getenv("TANDEM_OWNER")
	}
	if owner == "" {
		owner = "Someone"
	}
...
var toolDefs = []map[string]interface{}{
	{
		"name":        "join_meeting",
		"description": "Join a Google Meet call as the user's AI presence (e.g. \"Vatsa's Claude\"). Launches a headless Chrome that joins audio-only. After joining, call wait_for_turn to listen.",
		"inputSchema": map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"owner":      map[string]interface{}{"type": "string", "description": "The person you represent, e.g. \"Vatsa\". Defaults to TANDEM_OWNER env."},
				...
				"answers": map[string]interface{}{
					...
					"items": map[string]interface{}{
						...
						"properties": map[string]interface{}{
							...
							"topic":  map[string]interface{}{"type": "string", "description": "The question or topic this answers, used for routing (e.g. \"what is tandem / what are you building\")."},
