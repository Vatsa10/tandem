// Tandem desktop — a native macOS app that sets up and drives the tandem voice agent
...
// the whole app. Backgrounded work around the spawned tandem engine (broken pipes,
...
globalThis.addEventListener("unhandledrejection", (e) => {
  console.error("[tandem] unhandled rejection:", e.reason);
  e.preventDefault();
});
globalThis.addEventListener("error", (e) => {
  console.error("[tandem] uncaught error:", (e as ErrorEvent).error ?? e);
  e.preventDefault();
});
...
async function api(req: Request, pathname: string): Promise<Response | null> {
  switch (pathname) {
    case "/api/health":
      return Response.json({ ok: true, app: "tandem", version: "0.1.0" });
    case "/api/status":
    ...
    case "/api/tandem/build":
      return Response.json(await buildTandem());
...
Deno.serve({
  ...serveOpts,
  // Without a native window (Windows/Linux, or plain browser dev), open the UI
  // in the default browser once the server is listening. `deno desktop` sets
  // its own window, so skip the auto-open there.
  onListen: ({ hostname, port }) => {
    const url = `http://${hostname === "0.0.0.0" ? "localhost" : hostname}:${port}/`;
    console.log(`[tandem] UI at ${url}`);
    if (!underDesktop && Deno.env.get("TANDEM_NO_OPEN") !== "1") {
      openInShell(url).catch(() => {});
    }
  },
}, async (req) => {
...
function applicationMenu() {
  ...
  return [
    { submenu: { label: "Tandem", items: [role("about"), "separator", role("hide"), role("quit")] } },
    {
      submenu: {
        label: "Go",
        items: [
    ...
    { submenu: { label: "Help", items: [item("Tandem Help & Shortcuts", "help", "?")] } },
  ];
}
...
if (desktop.BrowserWindow) {
  ...
  win.addEventListener?.("menuclick", (e) => {
    ...
    else if (id === "join") win.executeJs?.(`globalThis.tandemOpenJoin&&globalThis.tandemOpenJoin()`);
    else if (id === "help") win.executeJs?.(`globalThis.tandemOpenHelp&&globalThis.tandemOpenHelp()`);
  });
}
