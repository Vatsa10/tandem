// Tandem desktop UI — vanilla, no build step. Talks to the Deno backend over the
...
// ---- Router -------------------------------------------------------------
function setWinTitle(suffix) { document.title = suffix ? "Tandem · " + suffix : "Tandem"; }

function show(view) {
  $$(".view").forEach((v) => v.classList.toggle("active", v.dataset.view === view));
  $$("#nav button").forEach((b) => b.classList.toggle("active", b.dataset.view === view));
  location.hash = view;
  if (["home", "integrations", "notes", "settings"].includes(view)) localStorage.setItem("tandem_view", view);
...
const TOOLS = [
  ...
  { id: "claude", name: "Claude", desc: "Let Claude send Tandem into a call.", status: "on" },
  { id: "codex", name: "Codex", desc: "Let Codex send Tandem into a call.", status: "on" },
...
const INT_META = {
  meet: { title: "Google Meet", desc: "Tandem joins a Meet call and talks for you." },
  calendar: { title: "Google Calendar", desc: "Your upcoming events, with one-tap join." },
  claude: { title: "Claude", desc: "Register Tandem as an MCP server for Claude." },
  codex: { title: "Codex", desc: "Register Tandem as an MCP server for Codex." },
...
function openIntegration(id) {
  ...
  if (id === "meet") {
    ...
    body = `<p class="int-note">${c ? "Tandem is reading your Google Calendar. Upcoming events appear on Home." : "Sign in with Google for read-only calendar access. Tandem is an unverified test app, so choose <b>Advanced → Continue</b>, then Allow."}</p>`;
    actions = c
    ...
    body = `<p class="int-note">${id === "claude"
      ? 'Adds a <span class="kbd">tandem</span> entry to Claude Desktop and runs <span class="kbd">claude mcp add</span>.'
      : 'Writes <span class="kbd">[mcp_servers.tandem]</span> to <span class="kbd">~/.codex/config.toml</span>.'}</p>`;
    actions = inst
    ...
    body = `<p class="int-note">Saved transcripts live in <span class="kbd">~/.tandem/notes</span>.</p>`;
...
// Install the tandem MCP server into a specific host (Claude or Codex).
...
globalThis.tandemOpenJoin = openJoin; // native menu → Join
...
// Normalize a raw speaker label: "test's Claude" / "participant" -> Tandem / You.
function speakerLabel(spk) {
  if (/tandem|claude/i.test(spk)) return { name: "Tandem", tandem: true };
  if (/participant|the meeting/i.test(spk)) return { name: "Them", tandem: false };
  return { name: spk || "Them", tandem: false };
}
function renderTranscript(raw) {
  ...
  box.innerHTML = out.map((e) => {
    ...
    return `<div class="line"><span class="spk ${l.tandem ? "tandem" : ""}">${esc(l.name)}</span>${esc(e.text)}</div>`;
  }).join("");
...
async function pollSession() {
  try {
    ...
    if (!s.active) {
      stopSessionPolling();
      // Session ended unexpectedly (tandem exited): surface it with a rejoin path.
      const err = $("#sessError");
      const tail = (s.logs || []).slice(-3).join(" · ") || "the session ended";
      err.innerHTML = `Tandem left the meeting. <span class="muted">${esc(tail)}</span> <button class="btn tiny" id="btnRejoin">Rejoin</button>`;
    ...
    if (tlen !== sessLastLen) {
      ...
      const botSpeaking = /tandem|claude/i.test(lastSpk);
...
$("#btnSaveNotes").addEventListener("click", async () => {
  ...
  toast(d.ok ? "Saved to ~/.tandem/notes" : (d.error || "Couldn't save"));
});
let muted = false;
...
function renderReadiness() {
  ...
  const rows = [
    ...
    ["tandem engine", STATUS.hasBinary, STATUS.hasBinary ? "Built" : '<button class="btn tiny" id="btnBuild">Build now</button>'],
  ...
  if (bb) bb.addEventListener("click", async (e) => {
    ...
    try {
      const d = await (await fetch("/api/tandem/build", { method: "POST" })).json();
...
// ---- Google Calendar ----------------------------------------------------
// Tandem ships a built-in Google OAuth client, so the only step is Connect Google.
...
globalThis.tandemOpenCalendar = openCalendar;
...
function closeOnboard() { $("#onboard").classList.remove("show"); localStorage.setItem("tandem_onboarded", "1"); }
...
globalThis.tandemOpenHelp = openHelp; // native menu → Help
...
if (location.search.includes("noob")) localStorage.setItem("tandem_onboarded", "1");
...
(async () => {
  ...
  else show(h || localStorage.getItem("tandem_view") || "home");
  // First run: no key saved and never onboarded → guided setup.
  if (h !== "onboard" && !localStorage.getItem("tandem_onboarded") && !STATUS.hasOpenAI) {
    // give status a beat to load, then decide
    setTimeout(() => { if (!STATUS.hasOpenAI && !localStorage.getItem("tandem_onboarded")) openOnboard(); }, 400);
  }
