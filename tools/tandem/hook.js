(function () {
  ...
  navigator.mediaDevices.getUserMedia = function (constraints) {
    ...
    // PulseAudio tandem_mic that carries our TTS) and supply the shader canvas as
      ...
      return origEnum().then(function (devices) {
        ...
        if (!hasCam) {
          devices = devices.concat([
            {
              deviceId: "tandem-entity-cam",
              kind: "videoinput",
              label: "Tandem Camera",
              groupId: "tandem-entity",
              toJSON: function () {
                return this;
              },
            },
  ...
  function _kajuEnsureTTSContext() {
    if (window._kajuAudioCtx) {
      // A context created before any user gesture (headless/automated Chrome)
      // starts "suspended", so its MediaStreamDestination emits pure silence.
      // Keep nudging it back to "running" or the bot is inaudible in the meeting.
      if (window._kajuAudioCtx.state === "suspended") {
        window._kajuAudioCtx.resume().catch(function () {});
      }
      return;
    }
    ...
    silence.start();
    window._kajuAudioCtx.resume().catch(function () {});
    console.log("[kaju] TTS AudioContext created, state=" + window._kajuAudioCtx.state);
  }
  ...
  function _kajuReplaceOutgoingTrack() {
    ...
    if (!ttsTrack) return;
    if (window._kajuOutTrack === ttsTrack) return; // already swapped in
    ...
    window._kajuPeerConnections.forEach(function (pc) {
      try {
        ...
        if (audioSender) {
          audioSender.replaceTrack(ttsTrack);
          replaced++;
        } else {
          // No audio sender yet (bot published no mic): add the TTS track so the
          // meeting actually receives our voice.
          try { pc.addTrack(ttsTrack, window._kajuTTSDest.stream); added++; } catch (e) {}
        }
      } catch (e) { console.log("[kaju] replaceTrack err: " + e); }
    });
    if (replaced || added) {
      window._kajuOutTrack = ttsTrack;
      console.log("[kaju] outgoing TTS track wired: replaced=" + replaced +
        " added=" + added + " pcs=" + window._kajuPeerConnections.length +
        " ctx=" + (window._kajuAudioCtx && window._kajuAudioCtx.state));
    }
  }
