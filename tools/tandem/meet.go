...
// capture, and its mic is a virtual source fed by TTS. On Windows/macOS it uses
// headed Chrome with in-browser audio (no OS audio plumbing).
...
func launchChromium(chrome, userDataDir, display string) (string, *exec.Cmd, error) {
	args := []string{
		...
		// Audio routes to tandem_out/tandem_mic via PULSE_SINK/PULSE_SOURCE env below.
	...
	// which is the only invocation that keeps Chromium alive under tandem (PID 1).
	logf, err := os.CreateTemp("", "tandem-chrome-*.log")
	if err != nil {
		return "", nil, err
	}
	logPath := logf.Name()
	...
	cmd.Env = append(os.Environ(),
		...
		"PULSE_SINK=tandem_out",
		"PULSE_SOURCE=tandem_mic",
	)
	cmd.Stdin = nil // /dev/null
	cmd.Stdout = logf
	cmd.Stderr = logf
	setDetached(cmd) // detached session (Linux: setsid); no-op elsewhere
...
func chromePath() string {
	...
	// Windows install locations (env roots are empty on mac/linux, so harmless).
	...
	if goruntime.GOOS == "windows" {
		return "chrome.exe" // resolved via PATH
	}
	return "chromium"
}
...
func silenceWAV() (string, error) {
	path := filepath.Join(os.TempDir(), "tandem-silence.wav")
...
func (ms *MeetSession) Join(displayName string) error {
	userDataDir := os.Getenv("TANDEM_CHROME_PROFILE")
	...
	if ms.container {
		...
		// Native path (Windows/macOS): headed Chrome, all audio in-browser via the
	...
	// Container mode: the bot's mic is the tandem_mic virtual device (TTS). Keep it
