...
func deviceInput() string { return strings.TrimSpace(os.Getenv("TANDEM_AUDIO_INPUT")) }

// sckHelperPath locates the compiled ScreenCaptureKit helper (tandemaudio), or ""
// if not found. Checks TANDEM_SCK_HELPER, then next to the binary, then ./mac.
func sckHelperPath() string {
	if p := strings.TrimSpace(os.Getenv("TANDEM_SCK_HELPER")); p != "" {
...
type Engine struct {
	meet      *MeetSession
	botName   string   // display name, e.g. "Vatsa's Claude"
	owner     string   // e.g. "Vatsa"
	...
	realtime      *RealtimeSession // "Mode A" audio-native backend (TANDEM_VOICE=realtime); nil otherwise
...
// Start wires up STT and launches the polling loops. If TANDEM_AUDIO_INPUT names a
...
func (e *Engine) Start() error {
	...
	// the tandem_out sink monitor; TTS goes out through the tandem_mic virtual mic. No
	...
	if pulseAvailable() {
		...
		if os.Getenv("TANDEM_VOICE") == "realtime" {
		...
		log.Printf("[engine] STT via PulseAudio (tandem_out.monitor), TTS via tandem_mic")
		return nil
	}
	...
	if os.Getenv("TANDEM_VOICE") == "realtime" && os.Getenv("OPENAI_API_KEY") != "" {
		go e.pollParticipants()
	...
	// 2) ScreenCaptureKit system-audio helper (opt-in via TANDEM_STT=sck).
	if os.Getenv("TANDEM_STT") == "sck" {
