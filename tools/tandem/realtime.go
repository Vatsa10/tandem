...
func realtimeModel() string {
	if m := os.Getenv("TANDEM_REALTIME_MODEL"); m != "" {
		return m
	}
	// Proven-working default. Set TANDEM_REALTIME_MODEL=gpt-realtime-2.1 for the
...
// whisper-1 (known-good); set TANDEM_REALTIME_TRANSCRIBE=gpt-realtime-whisper to try
// the newer, better entity-capture transcriber once verified in your account.
func realtimeTranscribe() string {
	if m := os.Getenv("TANDEM_REALTIME_TRANSCRIBE"); m != "" {
		return m
	}
	return "whisper-1"
}
...
func realtimeVoice() string {
	if v := os.Getenv("TANDEM_REALTIME_VOICE"); v != "" {
		return v
	}
	return "marin"
}
...
func realtimeEffort() string {
	if e := os.Getenv("TANDEM_REALTIME_EFFORT"); e != "" {
		return e
	}
	return "low"
}
...
func startRealtime(ctx context.Context, speaker *PulseSpeaker, persona string) (*RealtimeSession, error) {
	...
	if apiKey == "" {
		return nil, fmt.Errorf("OPENAI_API_KEY not set (required for TANDEM_VOICE=realtime)")
	}
	model := realtimeModel()
...
// startCapture streams tandem_out.monitor (the meeting audio) as 24kHz PCM into the
...
func (rs *RealtimeSession) startCapture() error {
	...
	rs.parec = cmd
	log.Printf("[realtime] capturing tandem_out.monitor -> realtime")
